    ...
    <div className="min-h-screen bg-[#F8F4EF] flex">
      <div className="hidden lg:flex flex-col justify-between w-[44%] border-r border-[#E4DDD6] p-12 bg-white">
        ...
        <div className="animate-fade-up">
          <p className="text-[10px] tracking-[0.3em] uppercase mb-5" style={{ color: A }}>{t('driverLogin.portalLabel')}</p>
          <h1 className="font-display text-[#1A1714] text-6xl font-light leading-tight mb-6">
            {t('driverLogin.hero1')}<br /><em className="italic">{t('driverLogin.hero2')}</em><br />{t('driverLogin.hero3')}
          </h1>
          <p className="text-[#6B5E52] text-sm leading-relaxed max-w-xs">{t('driverLogin.heroDesc')}</p>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div className="w-full max-w-sm animate-fade-up">
          ...
          <p className="text-[10px] tracking-[0.3em] uppercase mb-2" style={{ color: A }}>{t('driverLogin.portalSubLabel')}</p>
          ...
          <p className="text-[#A8998A] text-xs text-center mt-8">
            {t('common.noAccount')}{' '}
            <Link href="/driver/register" className="hover:underline" style={{ color: A }}>{t('driverLogin.registerLink')}</Link>
          </p>
        </div>
      </div>
    </div>
